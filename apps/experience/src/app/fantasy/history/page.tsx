...
function computeSummary(entries: typeof FANTASY_MOCK_HISTORY) {
  ...
  const avgRank = Math.round(entries.reduce((sum, e) => sum + e.overallRank, 0) / entries.length);
      ...
      {summary && (
        <FantasyPageHero
          ...
          stats={[
            ...
            { label: 'Best GW',      value: String(summary.bestGw.points) },
            { label: 'Avg Rank',     value: `#${summary.avgRank.toLocaleString()}` },
          ]}
        />
      )}
