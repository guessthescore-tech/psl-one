...
export default function GameweekDetailPage({ params }: GameweekDetailPageProps) {
  ...
  const entry = FANTASY_MOCK_HISTORY.find(e => e.gameweekId === gameweekId) ?? FANTASY_MOCK_HISTORY[0];
  ...
  return (
    <FantasyShell back={{ href: '/fantasy/history', label: 'Season History' }}>
      ...
      >
        <p className="text-label-lg text-exp-muted uppercase tracking-widest mb-1">{entry.gameweekName}</p>
        ...
        <div className="flex gap-6 mt-4 pt-4 border-t border-exp-border-dk">
          <div>
            <div className="text-stat-md text-white font-black">#{entry.overallRank.toLocaleString()}</div>
          ...
          <div>
            ...
            <div className="text-label-sm text-exp-muted">GW rank</div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="px-4 py-5 pb-20 flex flex-col gap-5">
        ...
        <div className="rounded-card bg-exp-navy border border-exp-border-dk p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-body-sm font-semibold text-white">
              {entry.transfersMade} transfer{entry.transfersMade !== 1 ? 's' : ''} made
            </div>
            <div className="text-label-sm text-exp-muted">
              {entry.transfersMade > 1 ? `−${(entry.transfersMade - 1) * 4} pts hit` : 'No point hit'}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <RivalTeamPitchView
            players={FANTASY_MOCK_TEAM.players}
            bench={FANTASY_MOCK_TEAM.bench}
            captainId={FANTASY_MOCK_TEAM.players[9]?.id}
          />
        </div>
