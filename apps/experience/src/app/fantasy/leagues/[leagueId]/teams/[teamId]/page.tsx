...
export default function RivalTeamPage({ params }: RivalTeamPageProps) {
  ...
  const manager = FANTASY_MOCK_STANDINGS.find(m => m.id === teamId);

  const rivalName = manager?.name ?? 'John Doe';
  const rivalTeamName = manager?.teamName ?? 'Galácticos FC';
  const rivalPoints = manager?.totalPoints ?? FANTASY_MOCK_TEAM.totalPoints;
  const rivalGwPoints = manager?.gwPoints ?? FANTASY_MOCK_TEAM.gwPoints;
  const rivalRank = manager?.rank ?? FANTASY_MOCK_TEAM.rank;
