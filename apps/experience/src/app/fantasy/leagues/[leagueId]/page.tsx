    ...
    <FantasyShell back={{ href: '/fantasy/leagues', label: 'All Leagues' }}>
      ...
      <div className="bg-exp-navy border-b border-exp-border-dk px-4 py-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-label-sm text-exp-muted mt-1">
              {league.memberCount.toLocaleString()} members · World Cup 2026
            </p>
          </div>
          {league.inviteCode && (
            <motion.button
        ...
        <div className="flex gap-6 mt-4 pt-4 border-t border-exp-border-dk">
          <div>
            <div className="text-stat-md text-exp-gold font-black">{league.myRank.toLocaleString()}</div>
          ...
          <div>
            <div className="text-stat-md text-white font-black">{league.memberCount.toLocaleString()}</div>
            <div className="text-label-sm text-exp-muted">Members</div>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'about' && (
        <div className="px-4 py-5 flex flex-col gap-4">
          {[
            ...
            { label: 'Members',       value: league.memberCount.toLocaleString() },
            { label: 'Created By',    value: league.managedBy },
          ].map(({ label, value }) => (
