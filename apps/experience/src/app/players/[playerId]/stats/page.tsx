...
export default async function PlayerStatsPage({ params }: PageProps) {
    ...
    return (
      <div className="min-h-[100dvh] bg-exp-surface">
        ...
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-10 space-y-8">
          <div>
            <div className="text-label-sm text-exp-gold font-bold uppercase tracking-wider mb-1">Season Statistics</div>
          ...
          <section aria-label="All season stats">
            <h2 className="text-label-lg text-exp-muted font-bold uppercase tracking-wider mb-3">Performance</h2>
            <PlayerStatGrid stats={allStats} columns={4} />
          </section>

          <section aria-label="Performance radar">
            <h2 className="text-label-lg text-exp-muted font-bold uppercase tracking-wider mb-3">Radar</h2>
            <div className="bg-exp-navy rounded-card border border-exp-border-dk p-6 flex flex-col items-center gap-4">
              ...
              <p className="text-label-sm text-exp-muted italic">Live radar chart not yet exposed by the API</p>
            </div>
          </section>

          <section aria-label="Match by match stats">
            <h2 className="text-label-lg text-exp-muted font-bold uppercase tracking-wider mb-3">Match by match</h2>
            <div className="bg-exp-navy rounded-card border border-exp-border-dk p-4">
              <PlayerGameweekTable
                rows={[
                  { gameweek: 1, label: 'GW 1', opponent: 'vs Live data', points: player.fantasyPoints, goals: player.goalsThisTournament, assists: player.assistsThisTournament, minutesPlayed: 90, rating: 8.1 },
                ]}
              />
            </div>
          </section>

          <section aria-label="Fantasy points breakdown" className="bg-exp-navy rounded-card border border-exp-gold/20 p-5">
            <div className="flex items-center gap-2 mb-4">
              <span aria-hidden>⚡</span>
              <h2 className="text-label-lg text-exp-gold font-bold uppercase tracking-wider">Fantasy Points Breakdown</h2>
            </div>
  ...
  const player = playerProfileToExpPlayer(profile, priceMap.get(profile.id));
  ...
  const allStats: PlayerStat[] = [
    ...
    { label: 'Yellow Cards', value: seasonSummary.totals.yellowCards },
    { label: 'Red Cards', value: seasonSummary.totals.redCards },
    { label: 'Clean Sheets', value: seasonSummary.totals.cleanSheets },
    { label: 'Fantasy Pts', value: seasonSummary.totals.fantasyPoints, highlight: true },
  ];
  ...
  const fantasyBreakdown: PlayerStat[] = [
    ...
    { label: 'Assists', value: seasonSummary.totals.assists },
    { label: 'Clean Sheets', value: seasonSummary.totals.cleanSheets },
    { label: 'Minutes', value: seasonSummary.totals.minutesPlayed },
    { label: 'Cards', value: seasonSummary.totals.yellowCards + seasonSummary.totals.redCards },
  ];

  const rows = seasonSummary.matches.map((match, index) => ({
    gameweek: index + 1,
    label: `GW ${index + 1}`,
    opponent:
