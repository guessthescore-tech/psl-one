...
export function GameweekHistoryCard({ entry, index = 0 }: GameweekHistoryCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/fantasy/history/${entry.gameweekId}`}
        className={clsx(
        ...
        aria-label={`${entry.gameweekName} — ${entry.points} points, overall rank ${entry.overallRank.toLocaleString()}`}
      >
        <div className="flex items-center gap-4">
          {/* GW label */}
          <div className="flex-shrink-0 w-12 text-center">
            <div className="text-label-sm text-exp-muted uppercase tracking-widest">GW</div>
            <div className="text-stat-md text-white font-black">{entry.gameweekName.replace('Matchday ', '')}</div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="text-label-sm text-exp-muted">
              Overall #{entry.overallRank.toLocaleString()}
            </div>
            {entry.chipUsed && (
            ...
            {entry.transfersMade > 0 && (
              <div className="text-label-sm text-exp-muted mt-0.5">
                {entry.transfersMade} transfer{entry.transfersMade > 1 ? 's' : ''}
              </div>
            )}
          </div>
