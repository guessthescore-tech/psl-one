...
export function ManagerRow({ manager, isCurrentUser, leagueId, index }: ManagerRowProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/fantasy/leagues/${leagueId}/teams/${manager.id}`}
        className={clsx(
        ...
        aria-label={`${manager.name} — ${manager.teamName} — Rank ${manager.rank}`}
      >
        {/* Rank */}
        ...
        <div className="text-right flex-shrink-0">
          <div className="text-body-sm font-black text-white">{manager.gwPoints}</div>
