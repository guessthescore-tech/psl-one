...
import { getSeasonLeaderboard, type SeasonLeaderboardRow } from '@/lib/fantasy-rules-client';
...
export default function SeasonLeaderboardPage({ params }: { params: Promise<{ seasonId: string }> }) {
  ...
  return (
    <main className="max-w-lg mx-auto p-4">
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-gray-400">No scores yet for this season.</p>
      ) : (
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b text-left text-xs text-gray-500">
              ...
              <th className="pb-2 text-right">Net pts</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.fantasyTeamId} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="py-2 pr-2 text-right text-red-400 text-xs">
                  {r.transferCost > 0 ? `-${r.transferCost}` : '—'}
                </td>
