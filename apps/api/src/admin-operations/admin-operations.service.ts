...
export class AdminOperationsService {
  ...
  async getSeasonModuleReadiness(seasonId: string): Promise<{ seasonId: string; seasonName: string; modules: ModuleReadinessItem[] }> {
    ...
    const modules: ModuleReadinessItem[] = [
      ...
      {
        moduleKey: 'PSL_BETA_LAUNCH_READINESS',
        displayName: 'PSL Beta Launch Readiness (13-check gate)',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: [],
        warnings: ['Season must not be activated without explicit admin approval and PSL_ADMIN role'],
        recommendedAction: 'Review GET /admin/beta-launch/:seasonId/readiness. Create approval at POST /admin/beta-launch/:seasonId/approve once all 13 checks pass.',
      },
      {
        moduleKey: 'FRONTEND_BETA_READINESS',
        displayName: 'Frontend Beta Readiness (19-domain walkthrough)',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: [],
        warnings: [],
        recommendedAction: 'Review GET /admin/beta-launch/:seasonId/frontend. Walk through all 19 domain admin and fan pages before beta cohort invite.',
      },
      {
        moduleKey: 'DATA_BETA_READINESS',
        displayName: 'Data Readiness (seeded clubs, players, fixtures)',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: [],
        warnings: ['Seed data is provisional. Confirm PSL player prices, squad registrations, and fixture schedule before invite.'],
        recommendedAction: 'Review GET /admin/beta-launch/:seasonId/data. Re-run seed and verify at /admin/squad-import and /admin/fixture-import.',
      },
      {
        moduleKey: 'SECURITY_BETA_READINESS',
        displayName: 'Security & RBAC Readiness',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: [],
        warnings: ['JWT secrets must be rotated before production. Review at /admin/operations/security.'],
        recommendedAction: 'Review GET /admin/beta-launch/:seasonId/security. Confirm RBAC gates on all PSL_ADMIN routes.',
      },
      {
        moduleKey: 'OPERATIONS_BETA_READINESS',
        displayName: 'Operations & Infrastructure Readiness',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: [],
        warnings: ['AWS ECS, CloudFront, RDS must be provisioned before live beta. Review at /admin/operations/infrastructure.'],
        recommendedAction: 'Review GET /admin/beta-launch/:seasonId/operations. Follow PSL-BETA-LAUNCH-RUNBOOK.md for production readiness checklist.',
      },
      {
        moduleKey: 'BETA_COHORT_READINESS',
        displayName: 'Beta Cohort Management',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: false,
        blockers: [],
        warnings: [],
        recommendedAction: 'Create beta cohort at POST /admin/beta-launch/cohorts. Invite users at POST /admin/beta-launch/cohorts/:cohortId/members.',
      },
      {
        moduleKey: 'ROLLBACK_READINESS',
        displayName: 'Rollback Dry-Run & Runbook',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: [],
        warnings: [],
        recommendedAction: 'Review GET /admin/beta-launch/:seasonId/rollback-dry-run. Confirm rollback runbook at PSL-BETA-ROLLBACK-RUNBOOK.md before beta invite.',
      },
      {
        moduleKey: 'ACTIVATION_APPROVAL',
        displayName: 'Season Activation Approval Gate',
        status: 'BUILT_NOW',
        isCommercial: false, isPointsOnly: false, isProductionEnabled: false, isFoundational: true,
        blockers: ['Season activation is NOT performed in STORY-39. Approval records only.'],
        warnings: ['Approval does not activate the season. Activation requires explicit admin trigger in a future deployment window.'],
        recommendedAction: 'Create approval at POST /admin/beta-launch/:seasonId/approve. Status will be APPROVED not ACTIVATED. Actual activation is a separate controlled operation.',
      },
    ];
