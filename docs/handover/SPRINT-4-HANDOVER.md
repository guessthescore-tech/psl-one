...
| Route | Description |
|-------|-------------|
| `/predict` | Full prediction game with score steppers, sharing, challenge links, lock states, error/empty states. Persistence: localStorage (design review) or `GET /football/fixtures?status=SCHEDULED` (live). |
| `/predict/challenge` | Challenge creation page with how-it-works, score entry, share link generation. Persistence: URL params only (LOCAL_DEMO_CHALLENGE — not backend-persisted). |
| `/predict/challenge/accept` | Challenge acceptance page with challenger's score display, counter-prediction, WhatsApp reply. Persistence: URL params only (LOCAL_DEMO_CHALLENGE). |
| `/account/notifications` | Notification preferences page. PARTIAL — wired to `GET/PATCH /notifications/preferences` when authenticated; falls back to design defaults when unauthenticated or in DESIGN_REVIEW_DATA mode. |
...
| `apps/experience/src/app/layout.tsx` | Added `robots: noindex` meta in preview/design-review mode; added skip-nav link (WCAG 2.1 AA) |
...
| `apps/experience/src/app/fantasy/fixtures/page.tsx` | Replaced wired stub with FantasyEmptyState (wiring deferred) |
| `apps/experience/src/app/fantasy/points/page.tsx` | Replaced "Coming soon" with FantasyEmptyState |
| `apps/experience/src/app/fantasy/rules/page.tsx` | Replaced "Coming soon" with FantasyEmptyState |
| `apps/experience/src/app/fantasy/stats/page.tsx` | Replaced "Coming soon" with FantasyEmptyState |

### New Configuration

| File | Purpose |
|------|---------|
| `apps/experience/vercel.json` | Vercel deployment config (noindex, security headers, env vars). Note: `outputDirectory` is `.next` (relative to `apps/experience` rootDirectory in Vercel dashboard). |
...
| `pnpm --filter @psl-one/experience build` | ✅ PASS — 59 static pages generated |
...
| `pnpm audit` | ⚠️ 3 moderate pre-existing (postcss/uuid/protobufjs) — in packages/testing only, NOT in experience app |
...
| Scope | Count |
|-------|-------|
| Experience routes total (unique) | 52 |
| Static pages generated by build | 59 (includes SSG param expansions) |
| LIVE_BETA_DATA routes | 18 |
| PARTIAL routes | 5 |
| DESIGN_REVIEW_DATA routes | 18 |
| STATIC_CONTENT routes | 6 |
| DEFERRED_STUB routes | 2 |
| MISSING_BACKEND_CONTRACT routes | 7 (subset of DESIGN_REVIEW_DATA) |
...
| Challenge persistence | LOCAL_DEMO_CHALLENGE — URL params only; backend `ChallengesController` exists but not wired to frontend |
